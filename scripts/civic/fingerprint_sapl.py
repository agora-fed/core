#!/usr/bin/env python3
"""Fingerprint SAPL — ÁGORA #72 / ADR-0017.

Para os municípios de uma UF (da tabela `municipio_ibge`), testa as URLs-base candidatas e
descobre quais câmaras rodam SAPL (Interlegis). Grava o resultado em `civic_source` (UPSERT) —
o catálogo que o extrator consome. Probe concorrente (ThreadPool), só stdlib.

Uso:
    python3 scripts/civic/fingerprint_sapl.py --uf RS [--limit 50] [--db postgresql://localhost/dsoc_build]
    # emite /tmp/civic-fingerprint-<UF>.sql e aplica no --db.

Priorização por população (ADR-0017): rode primeiro SP, PR, SC, RS, RJ, ES, MG…
"""
...
from sapl_client import candidate_bases, is_sapl  # noqa: E402
...
def probe_municipio(uf: str, ibge: str, nome: str) -> dict:
    """Testa as bases candidatas; retorna o 1º hit SAPL, ou marca dead."""
    for base in candidate_bases(nome, uf):
        ok, count = is_sapl(base)
        if ok:
            return {"ibge": ibge, "nome": nome, "platform": "sapl", "base": base,
...
def emit_sql(uf: str, results: list[dict]) -> Path:
    out = Path(f"/tmp/civic-fingerprint-{uf}.sql")
    lines = ["BEGIN;"]
    for r in results:
...
def main() -> int:
    ...
    print(f"[{uf}] probando {len(munis)} municípios (workers={WORKERS})…", flush=True)
    ...
    with concurrent.futures.ThreadPoolExecutor(max_workers=WORKERS) as ex:
        ...
        for fut in concurrent.futures.as_completed(futs):
            ...
            results.append(r)
            if r["status"] == "ok":
                print(f"  ✓ SAPL {r['nome']} → {r['base']} ({r['count']} parlamentares)", flush=True)

    hits = [r for r in results if r["status"] == "ok"]
    print(f"[{uf}] SAPL detectado em {len(hits)}/{len(results)} câmaras", flush=True)
