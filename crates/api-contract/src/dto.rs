...
/// One recipient of a proposal (0537 — multi-office). The first in the list is the
/// PRIMARY recipient (the same as `ProposalDto::mandate_id`).
...
pub struct ProposalTargetDto {
    ...
    pub sphere: String,
    /// Delivery receipt for THIS office (the e-mail left the relay), when it has.
...
pub struct ProposalDto {
    ...
    /// Urgency level (0.25.0-fediverse). `comum` (default) or `urgente` — the latter
    /// exige `titulo_status ∈ (validated,verified)` do votante (Fatia D). Aditivo,
    /// backward-compatible: an older front-end ignores the field.
    #[serde(default = "default_urgencia")]
    pub urgencia: String,
    /// Lifecycle (`draft` | `published` | `clustered`). Draft = still under
    /// moderation (not publicly visible). Phase A of the TSE plan (0.26.0).
...
pub struct MandateDto {
    ...
    /// when it is a PLACEHOLDER generated by the platform (`@parlamento.democracia.social.br`) — never
    /// expomos placeholder como canal real. Publicly exposed by design — accountability contact.
    #[serde(default, skip_serializing_if = "Option::is_none")]
    pub public_email: Option<String>,
    /// True when the office has a REAL institutional channel (an e-mail that delivers). False
    /// when only the platform placeholder exists — citizens must NOT be told to press (the
    /// silence would be OURS, not the official's). Integrity (critical ADR): 92% of mandates
    ...
    /// auth-only `MyMandateDto.binding_level`). Drives the verified-binding badge.
    ///
...
pub struct ProfileDto {
    ...
    /// Electoral-registry status (0.25.0-fediverse): `unverified` (submitted, no cross-check),
    /// `validated` (TSE check digits OK), `verified` (TSE cross-check), or `None` (never
    /// registered). Signals Brazilian political citizenship — the gate for urgent agenda items.
    #[serde(default)]
